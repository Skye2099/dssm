...
from preprocessing.utils import slice_arrays, get_video_embedding_from_vid


class UserVideoDataset(Dataset):
    def __init__(self, x, y=None, feature_columns=None,video_name=True):
        self.x_user_data = [x[feature] for feature in feature_columns]
        self.x_video_ids = x['id_x'].tolist()
        self.y_data = y
    
    def __getitem__(self, index):
        self.video_name = self.y_data
                # video_embedding logic
#         print('index for getitem:', index,'video_ids  is ',self.x_video_ids,)
        id = self.x_video_ids[index] 
        video_embedding = get_video_embedding_from_vid(id)
        video_embedding = video_embedding.squeeze()
#         print(video_embedding.shape)
        user_embedding = np.array(self.x_user_data).T[index]
#         user_embedding = torch.tensor(user_embedding)
#         print(index,video_embedding)
        if isinstance(self.y_data,np.ndarray):
            return ((user_embedding, video_embedding), self.y_data[index])
        else:
            return (user_embedding, video_embedding)
    
    def __len__(self):
        return len(self.x_video_ids)

class BaseTower(nn.Module):
    def __init__(self, user_feature_columns,l2_reg_embedding=1e-5,
        ...
        #
        self.feature_index = user_feature_columns
        #
        ...
        self.out = PredictionLayer(task,).cuda()
        self.to(device)
        #
    ...
    def fit(self, x=None, y=None, batch_size=None, epochs=1, verbose=1, initial_epoch=0, validation_split=0.,
        ...
        if isinstance(x, dict):
            train_tensor_data = UserVideoDataset(x,y, self.feature_index)
...
#             np.concatenate(x, axis=-1)),torch.tensor(total_embedding_list), torch.from_numpy(y))
        ...
        for epoch in range(initial_epoch, epochs):
            ...
            with tqdm(enumerate(train_loader), disable=verbose != 1,total=len(train_loader)) as t:
...
#                     print('y is:',y_train,'y_pred is :',y_pred)
                    optim.zero_grad()
...
#                     total_loss = loss + reg_loss + self.aux_loss
                    total_loss = loss 
                    print(f'step {_} loss is', total_loss)
...
#         )
        tensor_data = UserVideoDataset(x,feature_columns=self.feature_index)
...
#                 print('debug_2:',type(x_test),type(x_test[0]),type(x_test[1]),x_test[0].device,x_test[1].device)
                x = x_test
                y_pred = model(x).cpu().data.numpy()
#                 print('debug_3:',type(y_pred))
